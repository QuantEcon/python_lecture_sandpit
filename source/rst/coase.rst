...
Nevertheless, there exists a domain on which :math:`T` is well-behaved:  the set of convex increasing continuous functions :math:`p \colon [0,1] \to \mathbb R` such that :math:`c'(0) s \leq p(s) \leq c(s)` for all :math:`0 \leq s \leq 1`

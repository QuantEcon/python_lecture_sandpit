...
[add subcontracting fig here]

In this example,

* Firm 1 receives a contract to sell one unit of the completed good to a final buyer.  
  
* Firm 1 then forms a contract with firm 2 to purchase the partially completed good at stage :math:`t_1`, with the intention of implementing the remaining :math:`1 - t_1` tasks in-house (i.e., processing from stage :math:`t_1` to stage :math:`1`).  
...
As price takers, they face a price function :math:`p`, which is a map from :math:`[0, 1]` to :math:`\mathbb R_+`, with :math:`p(t)` interpreted as the price of the good at processing stage :math:`t`.  
...
(You might like to skip this secion on first reading.)
...
Nevertheless, there exists a domain on which :math:`T` is well-behaved:  the set of convex increasing continuous functions :math:`p \colon [0,1] \to \RR` such that :math:`c'(0) s \leq p(s) \leq c(s)` for all :math:`0 \leq s \leq 1`.  

We denote this set of functions by :math:`\mathcal P`.   

In :cite:`kikuchi2018span` it is show that the following statements are true:
...
#. the price function :math:`p^*` and this allocation together form an equilibrium for the production chain.
...
.. code-block:: python3
   ...
   def optimal_choices(pc, p_function, s):
       ...
       max of the minizer and zero.  This results in better results 
       close to zero.  

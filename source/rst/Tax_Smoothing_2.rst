...
By `Sebastian Graves <https://github.com/sebgraves>`__ and `Thomas J. Sargent <http://www.tomsargent.com/>`__
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
Barro’s 1979 model is about a government that borrows and lends in order
to help it minimize an intertemporal measure of distortions caused by
taxes. Technically, Barro’s 1979 model looks a lot like a consumption
...
Here :math:`w_{t+1} \sim {\cal N}(0,I)` and $:raw-latex:`\Pi`\_{ij} $ is
...
$. Therefore, the overall Q matrix for the Markov jump LQ problem is:
...
1. Use **LQ_markov_mapping** to map
   ...
   :math:`A,B,C,R,Q,W` matrices for each of the n states of the world.
...
large “long-short” positions in debt of different maturities. To prevent
such an outcome, we now set :math:`c_1 = 0.01`. This penalty is enough
...
In terms of dimensions, the first two matrices defined above are $ (H-1)
:raw-latex:`\times `H$. The last is :math:`1 \times H`.
...
To simplify the notation, let $S_t = \\begin{bmatrix}
(:raw-latex:`\tilde `S_x + p_t’S_s’S_x) & Ug \\end{bmatrix} $.

...
-  that the government wants to minimize the following measure of tax
   distortions: $E_0
   :raw-latex:`\sum`\_{t=0}\ :sup:`:raw-latex:`\infty `:raw-latex:`\beta``\ t
   T_t^2 $, where :math:`T_t` are total tax collections and :math:`E_0`
...
We let $ s(t) :raw-latex:`\equiv `s_t
:raw-latex:`\in [1, 2, \ldots, N]`$ be a time t realization of an
...
:math:`i = 1, \ldots, n` satisfy the :math:`N` interrelated Bellman
equations
...
:raw-latex:`\rho`(s_t) = :raw-latex:`\rho`\_i, i = 1,
:raw-latex:`\ldots`, n $ satisfy the following stacked system of
...
To find the appropriate (R,Q,W) matrices, we note that :math:`G_t` and
...
If we assume that :math:`p_{t,t+1} = \beta`, then $M_t
:raw-latex:`\equiv `M = - :raw-latex:`\beta `$. In this case, none of
...
our Q and W matrices will now also vary over time. We can solve such a

...
.. math::
  ...
  I_t = b (Y_{t-1} - Y_{t-2}) + \sigma \epsilon_{t}
...
Equations :eq:`consumption`, :eq:`investment`, and :eq:`income` imply the following second-order
...
Lets use these matrices to create an instance of the ``LinearStateSpace``
...
Notice, we didn't give the class a value for :math:`\Sigma_0`
...
.. code-block:: python3
    ...
    axes[0].plot(y[0:2, :].T)
    axes[0].set(xlabel='t', title='Simulation of $C_t$ and $Y_t$')
    axes[0].legend(['$Y_t$', '$C_t$'], loc='lower right')    
    axes[1].plot(y[2, :])
    axes[1].set(xlabel='t', title='Simulation of $I_t$')
    plt.show()
...
A useful method for answering this question is
``replicate()``
...
The following function checks the eigenvalues of the A matrix of
a linear state space model

If all eigenvalues of A have moduli strictly
